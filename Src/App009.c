...
void Send_Recv()
{
    char data[] = "Hi! USART Interrupt Working!";

    uint16_t len = strlen(data) + 1;

    // USART3 irq initialization and recv interrupt initialization
    USART_IRQInterruptConfig(USART3_IRQ, ENABLE);

    while (1)
    {
        wait_For_Btn_Press();
        USART_ReceiveDataIT(&usart3Handle, (uint8_t *)rdata, len);
...
{
    if (AppEv == USART_CALLBACK_DATA_RECV_COMPLETED)
    {
        char data[100];
        strcpy(data, rdata);
    }
