...
{
    SPI_IRQHandling(&spiMaster);
}

void SPI2_IRQHandler(void)
{
    SPI_IRQHandling(&spiSlave);
}

void SPI_AppCallback(SPI_Handle_t *pHandle, uint8_t status)
{
    if (status == CALLBACK_RX_COMPLETE)
    {
        data_available = 1;
    }
}
...
void send_data(SPI_Handle_t *spiMaster, SPI_Handle_t *spiSlave, uint8_t *data, uint8_t data_len, uint8_t *data_buf)
{
    ...
    while (SPI_SendWithIT(spiMaster, data, data_len) == BUSY_STATUS)
        ;

    // slave recv data byte length first from master
    while (SPI_ReceiveWithIT(spiSlave, data_buf, data_len) == BUSY_STATUS)
        ;
...
void SPI1_Inits()
{
    ...
    SPI_Init(&spiMaster);
    SPI_IRQConfig(SPI1_IRQ, 4, ENABLE);
    ...
    SPI_Init(&spiSlave);
    SPI_IRQConfig(SPI2_IRQ, 5, ENABLE);
